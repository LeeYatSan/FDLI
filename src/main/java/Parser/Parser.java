...
public class Parser
{
    ...
    {
        int i;
        for (i = 0; i < indent; ++i) ErrorLog.printLog(" ");
        ErrorLog.printLog("enter in " + x + "\n");
        indent += 2;
    }
    ...
    private TokenType FetchToken() // 获取记号
    {
        ...
        while (true)
        {
            ...
            {
                ErrorLog.printLog("discard ERRtext \" " +curr_token.getLexeme() + "\" at [%d" + curr_token.getPosition().getRow() + "," + curr_token.getPosition().getCol() + "]!\n");
                SyntaxError(1); //一类错误
                continue;
            }
            break;
    ...
    private void MatchToken(TokenType expected) // 匹配记号
    {
        ...
        {
            for (int i = 0; i < indent; ++i) ErrorLog.printLog(" ");
            ErrorLog.printLog("matchtoken "+ curr_token.getLexeme() +"\n");
            FetchToken();
        }
        else
        {
            ...
            {
                SyntaxError(2);  //二类错误
                ErrorLog.printLog("discard mismatchtoken \"" +curr_token.getLexeme() +"\" at [" + curr_token.getPosition().getRow() + "," + curr_token.getPosition().getCol() + "]!\n");
                TokenType tk = FetchToken();
                if (tk == expected)
                {
                    ErrorLog.printLog("*** matchtoken " + curr_token.getLexeme() + " after discard~~~\n");
                    lastToken = curr_token;
                    FetchToken();
                    break;
                }
    ...
    private void SyntaxError (int case_of)// 指出语法错误 ( 调用 error_msg)
    {
        ...
        {
            case 1:
                ErrorLog.errorMessage(curr_token.getPosition().getRow(),
